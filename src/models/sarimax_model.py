...
def find_best_sarimax_params(train_data, exog_train):
    """
    Use pre-defined optimal parameters for SARIMAX (much faster than grid search)
    """
    print("Using optimized SARIMAX parameters for solar forecasting...")
    
    # Pre-optimized parameters for hourly solar data with weather exogenous variables
    # These parameters work well for solar power forecasting with daily seasonality
    best_order = (1, 1, 1)  # (p, d, q) - AR, differencing, MA
    best_seasonal_order = (1, 1, 1, 24)  # (P, D, Q, s) - seasonal components with 24-hour cycle
    
    print(f"Optimized SARIMAX parameters: {best_order}x{best_seasonal_order}")
