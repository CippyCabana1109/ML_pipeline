...
# If your Weather_Data.csv is somewhere else, update the path above.
# The pipeline searches: data/Weather_Data.csv, Weather_Data.csv, data/Weather_Data_Clean.csv
...
if os.path.exists(NASA_RAW_PATH):
    ...
    # Try common locations for the NASA file
    alt_paths = ['Weather_Data.csv', 'data/Weather_Data_Clean.csv', 'Weather_Data_Clean.csv']
    nasa_found = None
    for p in alt_paths:
        if os.path.exists(p):
            nasa_found = p
    ...
    if nasa_found:
        print(f"  Found NASA file at: {nasa_found} — building physics dataset …")
        full_df = build_dataset_from_nasa(nasa_found)
        train = full_df[full_df['MO'] <= 11].copy()
        test  = full_df[(full_df['MO'] == 12) & (full_df['DY'] <= 7)].copy()
    else:
        print(f"  ⚠️  NASA file not found. Falling back to pre-processed CSVs …")
        print(f"  ⚠️  Add Weather_Data.csv to your data/ folder for best results.")
        train = pd.read_csv(TRAIN_PATH, parse_dates=['timestamp'])
        test  = pd.read_csv(TEST_PATH,  parse_dates=['timestamp'])
        # Inject realistic noise (old CSVs have synthetic power = irradiance×100)
        rng = np.random.default_rng(NOISE_SEED)
        for df_tmp in [train, test]:
            dt = df_tmp['irradiance'] > 0
            n  = np.ones(len(df_tmp))
            n[dt] = 1.0 + rng.normal(0, 0.12, dt.sum())
            df_tmp['solar_power_w'] = np.clip(df_tmp['solar_power_w'] * n, 0, None)
        # Add missing columns that NASA path provides
        for df_tmp in [train, test]:
            df_tmp['clearness']  = (df_tmp['irradiance'] / (df_tmp['irradiance'].max() + 1e-9)).clip(0, 1)
            df_tmp['dni']        = df_tmp['irradiance'] * 0.85
            df_tmp['diffuse']    = df_tmp['irradiance'] * 0.15
            df_tmp['wind_speed'] = 3.0   # default 3 m/s
            df_tmp['pressure']   = 83.0  # Nairobi avg kPa
            df_tmp['lag_24h']    = df_tmp['lag_24h'].fillna(0)
            df_tmp['lag_48h']    = df_tmp['lag_48h'].fillna(0)
